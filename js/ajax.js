...
function ajax_elemental_set (params, text)
{
	...
	if (!elemental) return;
        if (elemental.type == 'select-one') {
	            //elemental.options.length = 0;
                    //opt = new Option('', 0, false, false);
                    //elemental.options[0] = opt;
                    //elemental.options[0].innerHTML = text;
                    elemental.options[0].innerHTML = '' + text;
        }else{
		elemental.innerHTML = '' + text;
		elemental.style.display = (elemental.innerHTML == '') ? 'none' : 'inline';
	}
}
...
function ajax_elemental_add (params, text)
{
	...
	if (!elemental) return;
        if (elemental.type == 'select-one') {
		elemental.options[0].innerHTML = '' + elemental.options[0].innerHTML + text;
        }else{
