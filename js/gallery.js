	...
	var G = {
					...
					<div id='gallery-image-container'> \
						<div id='gallery-controls'> \
							<div id='gallery-previous'> \
								<a href='#' id='gallery-previous-link'>&laquo; <u>П</u>ред</a> \
							</div> \
							<div id='gallery-next'> \
								<a href='#' id='gallery-next-link'><u>С</u>лед &raquo;</a> \
							</div> \
						</div> \
					</div> \
		...
		display: function () {
			...
			G.meta.slideUp(300, function () {
				...
				G.image_container.fadeOut('fast', function () {
					...
					*/
					img.src = G.images.eq(G.current_idx).find('img').attr('src').replace(/\/small\//, '/middle/');

					if (G.current_idx !== 0) {
						// pre-load prev img
						var p = new Image();
						p.src = G.images.eq(G.current_idx - 1).find('img').attr('src').replace(/\/small\//, '/middle/');
					}
					if (G.current_idx !== (G.images.length - 1)) {
						// pre-load next img
						var n = new Image();
						n.src = G.images.eq(G.current_idx + 1).find('img').attr('src').replace(/\/small\//, '/middle/');
					}
				});
			});
		},
		...
		showControls: function () {
			...
			$(document).bind('keydown.gallery', function (e) {
				if (!G.active) {
					if ((e.keyCode === 37 || e.keyCode === 71) && G.current_idx !== 0) {
						G.active = true;
						G.previous.trigger('click.gallery');
					}
					else if ((e.keyCode === 39 || e.keyCode === 67) && G.current_idx !== (G.images.length - 1)) {
						G.active = true;
		...
		showMeta: function () {
			...
			G.title.html(title);
			G.pages.html('Изображение ' + (G.current_idx + 1) + ' из ' + G.images.length);
