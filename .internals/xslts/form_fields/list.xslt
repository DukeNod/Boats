...
<xsl:template name="list_of_form_fields">
	...
	<xsl:param name="form" />
		...
		<div class="link">
			<div class="linkCommand">
				<a href="{/*/system/info/adm_root}{$essence}/insert?form={$form}&amp;back={/*/system/curr_url}" >
	...
	<xsl:when test="$node/*">
			...
			<tr class="listRowHead">
				...
				<th class="listCellHead listCellSplitR">
					<xsl:call-template name="list_head_with_sorters">
						...
						<xsl:with-param name="text">Символьный код</xsl:with-param>
					</xsl:call-template>
				</th>
				<th class="listCellHead listCellSplitR">
					<xsl:call-template name="list_head_with_sorters">
				...
				<th class="listCellHead listCellSplitR">
					<xsl:call-template name="list_head_with_sorters">
						...
						<xsl:with-param name="text">Тип данных</xsl:with-param>
					</xsl:call-template>
				</th>
				<th class="listCellHead">
					<xsl:call-template name="list_head_with_sorters">
						...
						<xsl:with-param name="text">Обязательное</xsl:with-param>
					</xsl:call-template>
				</th>
		...
		<xsl:for-each select="$node/*">
			...
			<tr>
				...
				<td class="listCellData listCellSplitR" align="center" id="hidden_required{id}">
				<xsl:variable name="turn_off"><xsl:choose>
					<xsl:when test="required=1">0</xsl:when>
					<xsl:otherwise>1</xsl:otherwise>
				</xsl:choose></xsl:variable>
					<a href="javascript: void(0);" onClick="toggleHidden('form_fields/ajax/toggle', {id}, {$turn_off}, {$row}, 'required')">
						<xsl:choose>
	...
	<xsl:otherwise>
		<div class="list listEmpty">Сейчас нет ни одного поля.</div>
	</xsl:otherwise>
	</xsl:choose>
