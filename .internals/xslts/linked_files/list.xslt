...
<xsl:template name="list_of_linked_files">
	...
	<xsl:if test="not($hide_links)">
		<div class="link">
			<div class="linkCommand">
				<a href="{/*/system/info/adm_root}linked_files/insert/for/{$uplink_type}/{$uplink_id}?back={/*/system/curr_url}" >
	...
	<xsl:when test="$node/*">
		...
		<form class="list" method="post" action="{/*/system/info/adm_root}linked_files/multi">
		<table class="listTable">
			...
			<tr class="listRowHead">
				...
				<th class="listCellHead" colspan="2">
					<xsl:call-template name="list_head_with_sorters">
						<xsl:with-param name="filters" select="$filters"/>
						<xsl:with-param name="sorters" select="$sorters"/>
		...
		<xsl:for-each select="$node/*">
			<tr>
				...
				<td class="listCellLink listCellMergeR">
					<xsl:call-template name="file_icon">
						<xsl:with-param name="dir" select="concat(/*/system/info/adm_root, 'img/icons/')"/>
						<xsl:with-param name="file" select="attach_file"/>
					</xsl:call-template>
				</td>
				<td class="listCellData listCellMergeL">
					<nobr>
						<a href="{/*/system/info/adm_to_dir_for_linked}files/attach/{uplink_type}/{uplink_id}/{attach_file}" target="_blank">
			...
			<xsl:if test="not($hide_links)">
				<td class="listCellLink listCellSplitL">
					<a href="{/*/system/info/adm_root}linked_files/update/{id}?back={/*/system/curr_url}">
					<img src="{/*/system/info/adm_root}img/buttons/update{$row}.gif" width="20" height="20" alt="изменить" /></a>
				</td>
				<td class="listCellLink">
					<a href="{/*/system/info/adm_root}linked_files/delete/{id}?back={/*/system/curr_url}">
					<img src="{/*/system/info/adm_root}img/buttons/delete{$row}.gif" width="20" height="20" alt="удалить" /></a>
				</td>
				<td class="listCellLink">
					<xsl:choose>
					<xsl:when test="position&gt;1">
						<a href="{/*/system/info/adm_root}linked_files/moveup/{id}?back={/*/system/curr_url}">
				...
				<td class="listCellLink listCellSplitR">
					...
					<xsl:when test="position&lt;count($node/*)">
	        				<a href="{/*/system/info/adm_root}linked_files/movedn/{id}?back={/*/system/curr_url}">
	...
	<xsl:otherwise>
		<div class="list listEmpty">Сейчас нет ни одного прикреплённого файла.</div>
	</xsl:otherwise>
	</xsl:choose>
