...
<xsl:template name="form_for_linked_file">
		...
		<xsl:if test="($form/data/attach_temp!='') or ($form/data/attach_file!='')">
			<tr class="formRowView">
				...
				<td class="formCellControls">
					<a target="_blank">
						<xsl:choose>
						<xsl:when test="$form/data/attach_temp!=''"><xsl:attribute name="href"><xsl:value-of select="concat(/*/system/info/adm_to_tmp_for_linked, $form/data/attach_temp)"/></xsl:attribute></xsl:when>
						<xsl:when test="$form/data/attach_file!=''"><xsl:attribute name="href"><xsl:value-of select="concat(/*/system/info/adm_to_dir_for_linked, 'files/attach/', $form/data/uplink_type, '/', $form/data/uplink_id, '/', $form/data/attach_file)"/></xsl:attribute></xsl:when>
						</xsl:choose>
...
<!--в этом проекте не используется, но сохранено чтоб можно было включить когда-ниьудь.
			<tr class="formRowOptional">
				<th class="formCellHead"><label for="{$prefix}preview"><nobr>Превью</nobr></label></th>
				<td class="formCellFlag"></td>
				<td class="formCellControls">
					<input class="controlAttach" type="file"   name="preview" id="{$prefix}preview"/>
					<input class="controlHidden" type="hidden" name="preview_temp" value="{$form/data/preview_temp}"/>
				</td>
			</tr>
		<xsl:if test="$form/errors/preview_exists">
			<tr class="formRowRecovery">
				<th class="formCellHead"><label for="{$prefix}preview_name"><nobr>Имя файла превью</nobr></label></th>
				<td class="formCellFlag">&gt;</td>
				<td class="formCellControls">
					<input class="controlString" type="text" name="preview_name" value="{$form/data/preview_name}" id="{$prefix}preview_name"/>
				</td>
			</tr>
		</xsl:if>
		<xsl:if test="($form/data/preview_temp!='') or ($form/data/preview_file!='')">
			<tr class="formRowView">
				<th class="formCellHead">
					<nobr>Превью</nobr>
					<br/>
					<input type="checkbox" name="preview_delete" value="1" id="{$prefix}preview_delete">
						<xsl:if test="number($form/data/preview_delete)&gt;0"><xsl:attribute name="checked"/></xsl:if>
					</input>
					<label for="{$prefix}preview_delete">Удалить</label>
				</th>
				<td class="formCellFlag"></td>
				<td class="formCellControls">
					<xsl:choose>
					<xsl:when test="$form/data/preview_temp!=''"><img src="{/*/system/info/adm_to_tmp_for_linked}{$form/data/preview_temp}"/></xsl:when>
					<xsl:when test="$form/data/preview_file!=''"><img src="{/*/system/info/adm_to_dir_for_linked}files/preview/{$form/data/preview_file}"/></xsl:when>
					</xsl:choose>
				</td>
			</tr>
		</xsl:if>
			...
			<tr class="formRowOptional">
				...
				<td class="formCellControls">
					<select class="controlSelect" name="position" id="{$prefix}position">
						...
						<xsl:for-each select="/*/enums[@for='linked_files']/siblings/*[id!=$form/id or not($form)]">
							<option value="&gt;{id}">
