...
<xsl:template name="view_for_subscriber">
		...
		<xsl:if test="$form/errors/*">
			...
			<tr class="formRowView">
				...
				<td class="formCellData">
					<nobr>
						<xsl:value-of select="$form/data/email" disable-output-escaping="no"/>
					</nobr>
				</td>
			</tr>
			<tr class="formRowView">
				<th class="formCellHead"><nobr>Код активации</nobr></th>
				<td class="formCellFlag"></td>
				<td class="formCellData">
					<nobr>
						<xsl:value-of select="$form/data/code"/>
					</nobr>
				</td>
			</tr>
			<tr class="formRowView">
				...
				<td class="formCellData">
					<xsl:choose>
					<xsl:when test="number($form/data/is_active)&gt;0">Подписка активна (подтверждена).</xsl:when>
					<xsl:otherwise>Подписка не активна (ещё не подтверждена либо приостановлена).</xsl:otherwise>
					</xsl:choose>
				</td>
			</tr>
			<tr class="formRowView">
				<th class="formCellHead"><nobr>Тестер</nobr></th>
				<td class="formCellFlag"></td>
				<td class="formCellData">
					<xsl:choose>
					<xsl:when test="number($form/data/is_tester)&gt;0">Получает и тестовые, и всеобщие рассылки.</xsl:when>
					<xsl:otherwise>Не получает тестовые рассылки (только всеобщие).</xsl:otherwise>
					</xsl:choose>
				</td>
			</tr>
