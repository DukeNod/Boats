...
<xsl:template name="form_for_page">
	...
	<form enctype="multipart/form-data" method="post" action="{$action}">
		...
		<xsl:if test="$form/errors/*">
			...
			<tr class="formRowRequired">
				<th class="formCellHead"><nobr>Родительский раздел</nobr></th>
				<td class="formCellFlag">*</td>
				<td class="formCellControl">
					<select class="controlSelect" name="parent">
						<option value="0">Верхний уровень</option>
						<xsl:apply-templates select="/*/enums//category_tree/page">
							<xsl:with-param name="id" select="$form/data/parent"/>
						</xsl:apply-templates>
					</select>
				</td>
			</tr>
			...
			<tr class="formRowOptional">
				<th class="formCellHead"><nobr><label for="{$prefix}public">Опубликован</label></nobr></th>
				<td class="formCellFlag"></td>
				<td class="formCellControls"><nobr>
					<input class="controlCheckbox" type="checkbox" name="public" value="1" id="{$prefix}public">
						<xsl:if test="number($form/data/public)&gt;0"><xsl:attribute name="checked"/></xsl:if>
					</input>
