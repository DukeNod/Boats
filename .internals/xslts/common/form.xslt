...
<xsl:template name="form_for_common_element">
	...
	<form id="main_form" enctype="multipart/form-data" method="post" action="{$action}">
		...
		<xsl:if test="$form/errors/*">
			...
			<xsl:apply-templates select="/*/module/fields/*" mode="admin_quick_form">
		...
		<input type="hidden" name="back" value="{$back}"/>
			...
			<script type="text/javascript">
				$(function(){ var prefix = '_';

					ajax_del_items(prefix, 'items_remove');
				});
			</script>
		<xsl:if test="/*/module/additional_js">
			<script type="text/javascript" src="{/*/system/info/adm_root}js/{/*/module/additional_js}"></script>
		</xsl:if>
</xsl:template>
