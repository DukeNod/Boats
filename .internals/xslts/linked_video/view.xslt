...
<xsl:template name="view_for_linked_video">
	...
	<form class="form" enctype="multipart/form-data" method="post" action="{$action}">
		...
		<xsl:if test="($form/data/attach_temp!='') or ($form/data/attach_file!='')">
			<tr class="formRowView">
				<th class="formCellHead"><nobr>Файл</nobr></th>
				<td class="formCellFlag"></td>
				<td class="formCellData">
					<a target="_blank">
						<xsl:choose>
			...
			<tr class="formRowView">
				<th class="formCellHead"><nobr>Размер</nobr></th>
				<td class="formCellFlag"></td>
				<td class="formCellData">
					<xsl:value-of select="$form/data/attach_size" disable-output-escaping="no"/>
				</td>
			</tr>
		</xsl:if>
