...
<xsl:include href="../modules.public/order/form.xslt"/>
...
<xsl:template match="/" mode="overrideable_title">
	<xsl:value-of select="/*/system/info/sitename"/><xsl:text> &mdash; заполнена форма заказа </xsl:text><xsl:value-of select="$now"/>
</xsl:template>
...
<xsl:template match="/" mode="overrideable_content">
	...
	<h2><xsl:value-of select="/*/system/info/sitename"/> &mdash; заполнена форма заказа <xsl:value-of select="$now"/></h2>
...
{
BORDER-RIGHT: 0px;
BORDER-TOP: #7A96C9 1px solid; 
BORDER-LEFT: #7A96C9 1px solid
}
.adm td, .adm th
{
BORDER-RIGHT: #7A96C9 1px solid;
BORDER-BOTTOM: #7A96C9 1px solid; 
BORDER-LEFT: 0px
}
	</style>
	
<table class="adm" cellspacing="0">
	<xsl:for-each select="/*/fields_order/*">
		<xsl:apply-templates select="./*" mode="quick_mail"/>
		<tr><td></td></tr>
	</xsl:for-each>
</table>
<br/>
