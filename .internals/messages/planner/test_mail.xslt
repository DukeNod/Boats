...
<xsl:template match="/" mode="overrideable_title">
	Тест почты из планнера
	<!--<xsl:value-of select="/*/system/info/sitename"/><xsl:text> &mdash; заполнена форма '</xsl:text><xsl:value-of select="/*/form/name"/><xsl:text>' </xsl:text><xsl:value-of select="$now"/>-->
</xsl:template>

<xsl:template match="/" mode="overrideable_content">
	<!--
	<style>
	table			{ border-collapse: collapse; border: solid 1px; margin-top: 20px; }
	td, th			{ vertical-align: top; text-align: left; padding: 0 0 0 0; border: solid 1px gray; }
	th				{ color: #7a96c9; }
	</style>
	-->

    <h2>Сообщение из планнера: <xsl:value-of select="$now" /></h2>

	<table style="border: 1px solid black; border-style: collapse; border-collapse: collapse">
		<xsl:for-each select="/*/mail/item">
	...
	<!--
	<h2><xsl:value-of select="/*/system/info/sitename"/> &mdash; заполнена форма '<xsl:value-of select="/*/form/name"/>' <xsl:value-of select="$now"/></h2>
