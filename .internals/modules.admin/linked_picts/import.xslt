...
<xsl:template match="/" mode="overrideable_redirect">
	<xsl:if test="/*/done">
		<xsl:value-of select="concat(/*/system/info/adm_root, /*/uplink/@sname, '/update/', /*/done/data/uplink_id)"/>
	</xsl:if>
</xsl:template>
...
<xsl:template match="/*/list">
	<xsl:call-template name="list_for_linked_pict_import">
		...
		<xsl:with-param name="button_w"   select="93"/>
		<xsl:with-param name="button_h"   select="19"/>
		<xsl:with-param name="action"   select="concat(/*/system/info/adm_root, 'linked_picts/import/for/', data/uplink_type, '/', data/uplink_id)"/>
	</xsl:call-template>
</xsl:template>
