...
<xsl:include href="../../xslts/meta/list.xslt"/>
...
<xsl:template match="/" mode="overrideable_title">
	<xsl:text>Перекрытие мета-тегов</xsl:text>
</xsl:template>

<xsl:template match="/" mode="overrideable_path">
	<xsl:call-template name="path_this"><xsl:with-param name="text" select="'Мета-теги'"/></xsl:call-template>
</xsl:template>



<xsl:template match="/*/list">
	<xsl:call-template name="list_of_meta">
