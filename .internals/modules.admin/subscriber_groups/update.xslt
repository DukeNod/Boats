...
<xsl:include href="../../xslts/_common_/langs.xslt"/>
...
<xsl:include href="../../xslts/subscriber_groups/list.xslt"/>
...
<xsl:template match="/" mode="overrideable_selector">
	<xsl:value-of select="concat($essence, /*/form/data/domain)"/>
</xsl:template>
...
<xsl:template match="/" mode="overrideable_path">
	<xsl:call-template name="path_link"><xsl:with-param name="link" select="concat(/*/system/info/adm_root, $essence, '?domain=', /*/form/data/domain)"/><xsl:with-param name="text" select="$essence_title"/></xsl:call-template>
