...
$defs   = array('group' => 'text');
$data   = array_merge_rol($defs, $info, $m->form_posted_full($action));
...
if (($action === 'do') && !count($errors)) $m->validate_full($errors, $data, $config, $enums);
if (($action === 'do') && !count($errors))   $m->insert_full($errors, $data, $id);
