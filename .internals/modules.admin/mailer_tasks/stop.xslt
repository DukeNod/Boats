...
<xsl:template match="/" mode="overrideable_title">
	<xsl:text>Останов задания для рассылки</xsl:text>
</xsl:template>
...
<xsl:template match="/" mode="overrideable_path">
	...
	<xsl:call-template name="path_this"><xsl:with-param name="text" select="'Останов'"/></xsl:call-template>
</xsl:template>
...
<xsl:template match="/*/form">
	...
	<xsl:call-template name="view_for_mailer_task">
		...
		<xsl:with-param name="button_alt" select="'Прервать рассылку'"/>
		<xsl:with-param name="button_img" select="'stop'"/>
